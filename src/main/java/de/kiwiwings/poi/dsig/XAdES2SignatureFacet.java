...
import static de.kiwiwings.poi.dsig.SignatureFacetHelper.newTransform;
 ...
 *
 */
public class XAdES2SignatureFacet implements SignatureFacet {

    private static final Logger LOG = LogManager.getLogger(org.apache.poi.poifs.crypt.dsig.facets.XAdESSignatureFacet.class);
    ...
    public void preSign(
        ...
        addCommitType(signatureInfo, signedProperties);
    ...
    private void addCommitType(SignatureInfo signatureInfo, SignedPropertiesType signedProperties) {
        ...
        ctid.setDescription("Created and approved this document");
        cti.addNewAllSignedDataObjects();
        String sigDesc = signatureInfo.getSignatureConfig().getSignatureDescription();
        if (sigDesc != null) {
            AnyType ctq = cti.addNewCommitmentTypeQualifiers().addNewCommitmentTypeQualifier();
            ctq.set(XmlString.Factory.newValue(sigDesc));
        }
    }
